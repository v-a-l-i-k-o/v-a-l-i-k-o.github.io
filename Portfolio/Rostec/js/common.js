...
function firstSlider(elem){
	...
	$(elem).on('translate.owl.carousel', function(event) {
		if(status){status=false; return false;}
		video.trigger('next.owl.carousel');
	});
...
function videoSlider(elem){
	...
	$(elem).on('translate.owl.carousel', function(event) {
		...
		if(body.width() > 430){
			...
			/*next.get(0).addEventListener("timeupdate", function() {
               if(!$(this).closest('.owl-item').hasClass('active') && this.currentTime > 1){
			...
			next.on( "ended", function(){
				if($(this).closest('.owl-item').hasClass('active'))firstslide.trigger('next.owl.carousel');
			});
		}else{
...
function newsOpen(elem){
    ...
    // custom NB!
    //History.replaceState({type: 'list'}, $(elem).data('list-title'), $(elem).data('list-url'));
};
...
function newsClose(elem){
    ...
    // custom NB!
    //History.replaceState({type: 'base'}, $(elem).data('base-title'), $(elem).data('base-url'));
};
...
function viewPhoto(elem){
	...
	function adaptiveImage(){
		setTimeout(function(){
			...
			$('.arthidden').height(h);
			$('.arthidden').css({'margin-top': "-"+parseInt((h/2))+'px'});
		},100);
	};
...
function emptySearchInput(elem){
	var input = $(elem),
			close = $('.empty-search');
...
// Скрываем годовой график

function hideGraph(elem){
	var graph = $('.years-line');
	$(elem).on('click', 'a', function(event) {
		if($(this).attr('href') == '#stat-tab2') {
...
$(document).ready(function(){
	...
	$('.mmenu, .events-news, .vert-scroll').mCustomScrollbar({ //, .news-artscroll
		axis                : 'y',
		scrollInertia       : 600,
    ...
    // NB! custom
    $('.news-artscroll').mCustomScrollbar({
		axis                : 'y',
		scrollInertia       : 600,
		mouseWheelPixels    : 160,
		autoHideScrollbar   : false,
    ...
    */
	...
	emptySearchInput('.form-group__search input[type="search"]');

	hideGraph('.stats-tabs');
	...
	*/
	...
	$(document).on('click', '.arthidden img.current', function(){
		...
		$('.artgallery .next').trigger('click');

	});

	$(document).on('click', '.closeart, .artoverlay', function(){
	...
	$('.years-list').on('click', 'li', function(event) {
		event.preventDefault();
		var target = $(this).attr('target');

		if(!target || $(this).hasClass('active') || $(this).hasClass('disabled') ){return true;}
		$(this).siblings('li').removeClass('active');
		$(this).addClass('active');

		$('#stat-tab1').addClass('anim');
		setTimeout(function(){
			$('#stat-tab1').removeClass('y2007 y2008 y2009 y2010 y2011 y2012 y2013 y2014 y2015').addClass(target);

			setTimeout(function(){
				$('#stat-tab1').removeClass('anim');
			},200);
		;},800);
	...
	$('.history-photo').bind('mousewheel DOMMouseScroll MozMousePixelScroll', function(event) {
		event.preventDefault();

		delta = parseInt(event.originalEvent.wheelDelta || -event.originalEvent.detail);

		var hcur = $('.history .history-years li.current');

		if (delta >= 0) {
		  hcur.prev("li").trigger('click');
		} else {
		  hcur.next("li").trigger('click');
		}
	});

    /* see initShareButtons()
    $(function() {
        if (typeof $(".share-button").ayoshare == 'function'){
            $(".share-button").ayoshare(
                    facebook = true,
                    vk = true,
                    twitter = true
            );
    ...
    */
	...
	if ( $(window).scrollTop() != 0 )
		$('html, body').animate({ scrollTop: $(window).scrollTop() - 120 }, 1);
